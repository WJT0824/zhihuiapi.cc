...
// Codes minted by already-released plugin builds are signed with this value, so
// it stays accepted as a fallback. Rotate through the admin API and turn
// compatibility off once every plugin build ships with the new secret.
const RECHARGE_SHORT_LEGACY_SECRET = 'ZHCDR-SHORT-2026-9F3A7C21B5D8E6';
const RECHARGE_SHORT_ENV_SECRET = String(process.env.ZH_RECHARGE_SHORT_SECRET || '').trim();
const RECHARGE_SHORT_ENV_LEGACY = String(process.env.ZH_RECHARGE_SHORT_SECRET_LEGACY || '').trim();
...
/**
 * Short-code signing keys, resolved in priority order:
 *   ZH_RECHARGE_SHORT_SECRET env -> value saved from the admin API -> built-in
 * The legacy key stays in the accepted list by default so codes generated by
 * plugin builds that are already in circulation keep redeeming.
 */
const shortCodeKeyring = () => {
  const stored = store.shortCode || {};
  const storedPrimary = String(stored.primary || '').trim();
  const primary = RECHARGE_SHORT_ENV_SECRET || storedPrimary || RECHARGE_SHORT_LEGACY_SECRET;
  const accepted = [primary];
  if (RECHARGE_SHORT_ENV_LEGACY) accepted.push(RECHARGE_SHORT_ENV_LEGACY);
  for (const value of Array.isArray(stored.legacy) ? stored.legacy : []) {
    const secret = String(value || '').trim();
    if (secret) accepted.push(secret);
  }
  const allowLegacy = stored.allowLegacy !== false;
  if (allowLegacy) accepted.push(RECHARGE_SHORT_LEGACY_SECRET);
  const secrets = [...new Set(accepted.filter(Boolean))];
  return {
    primary,
    secrets,
    allowLegacy,
    source: RECHARGE_SHORT_ENV_SECRET
      ? 'env'
      : storedPrimary
        ? 'store'
        : 'builtin',
    primaryIsLegacy: primary === RECHARGE_SHORT_LEGACY_SECRET,
  };
};
...
const shortRechargeSignature = (payload, secret) => {
  const digest = crypto.createHmac('sha256', String(secret || '')).update(String(payload || '')).digest();
  let bits = 0;
  let buffer = 0;
  let output = '';
...
const verifyShortRechargeCode = (code) => {
  ...
  const keyring = shortCodeKeyring();
  const matched = keyring.secrets.find((secret) => shortRechargeSignature(payload, secret) === signature);
  if (!matched) return null;
  return {
    points: RECHARGE_SHORT_POINTS[index],
    nonce: payload.slice(1),
    secretSource: matched === keyring.primary ? keyring.source : 'legacy',
  };
};
/** Mints a plugin-compatible short code using the active primary key. */
const createShortRechargeCodes = (points, count) => {
  const tier = RECHARGE_SHORT_POINTS.indexOf(Number(points));
  if (tier < 0) return null;
  const { primary } = shortCodeKeyring();
  const codes = [];
  for (let i = 0; i < count; i += 1) {
    let nonce = '';
    while (nonce.length < 7) nonce += RECHARGE_SHORT_ALPHABET[crypto.randomInt(0, RECHARGE_SHORT_ALPHABET.length)];
    const payload = RECHARGE_SHORT_ALPHABET[tier] + nonce;
    codes.push(`${RECHARGE_SHORT_CODE_PREFIX}${payload}${shortRechargeSignature(payload, primary)}`);
  }
  return codes;
};
...
async function gateway(req, res, pathName) {
  ...
  if (req.method === 'GET' && pathName === '/v1/admin/short-code') {
    const keyring = shortCodeKeyring();
    return send(res, 200, {
      success: true,
      status: {
        source: keyring.source,
        primaryPreview: maskSecret(keyring.primary),
        primaryIsBuiltin: keyring.primaryIsLegacy,
        allowLegacy: keyring.allowLegacy,
        acceptedKeys: keyring.secrets.length,
        tiers: RECHARGE_SHORT_POINTS,
        envManaged: Boolean(RECHARGE_SHORT_ENV_SECRET),
      },
    });
  }
  if ((req.method === 'PUT' || req.method === 'POST') && pathName === '/v1/admin/short-code') {
    store.shortCode ||= { primary: '', legacy: [], allowLegacy: true };
    if (body.primary !== undefined) {
      const primary = String(body.primary || '').trim();
      if (primary && primary.length < 12) return fail(400, '新密钥至少 12 位，建议使用 32 位以上随机字符串。');
      if (primary) {
        const legacy = new Set(Array.isArray(store.shortCode.legacy) ? store.shortCode.legacy : []);
        legacy.add(store.shortCode.primary || RECHARGE_SHORT_LEGACY_SECRET);
        store.shortCode.legacy = [...legacy].filter((value) => value && value !== primary);
      }
      store.shortCode.primary = primary;
    }
    if (body.allowLegacy !== undefined) store.shortCode.allowLegacy = Boolean(body.allowLegacy);
    await persist();
    const keyring = shortCodeKeyring();
    return send(res, 200, {
      success: true,
      message: RECHARGE_SHORT_ENV_SECRET && body.primary !== undefined
        ? '当前由环境变量 ZH_RECHARGE_SHORT_SECRET 指定主密钥，后台设置不会生效。'
        : `已保存。当前主密钥 ${maskSecret(keyring.primary)}，可接受 ${keyring.secrets.length} 个密钥${keyring.allowLegacy ? '（兼容旧版插件生成的兑换码）' : '（已关闭旧版兼容）'}。`,
      status: {
        source: keyring.source,
        primaryPreview: maskSecret(keyring.primary),
        primaryIsBuiltin: keyring.primaryIsLegacy,
  ...
  if (req.method === 'POST' && pathName === '/v1/admin/short-code/generate') {
    const points = Number(body.points || body.amount || 100);
    const count = Math.max(1, Math.min(200, Number(body.count) || 1));
    const codes = createShortRechargeCodes(points, count);
    if (!codes) return fail(400, `积分档位仅支持：${RECHARGE_SHORT_POINTS.join('、')}`);
    return send(res, 201, { success: true, points, count, codes, format: 'ZHRC1-short' });
  }
